...
export function calculateAssessmentScore(answers: Record<string, number>): AssessmentResult {
  ...
  const overallReadiness = Math.round(
    ((earthquakeScore + floodScore + generalScore) / (earthquakeTotal + floodTotal + generalTotal)) * 100
  )
...
function generateRecommendations(earthquakeScore: number, floodScore: number): string[] {
  const recommendations: string[] = []

  if (earthquakeScore < 60) {
    recommendations.push(
      '🏠 Tingkatkan kesiapan gempa: Amankan barang-barang yang bisa bergerak dan pelajari teknik drop, cover, hold on.'
    )
  }

  if (floodScore < 60) {
    recommendations.push(
      '💧 Tingkatkan kesiapan banjir: Periksa saluran drainase dan siapkan rencana evakuasi vertikal.'
    )
  }

  if (earthquakeScore >= 80 && floodScore >= 80) {
    recommendations.push(
      '⭐ Luar biasa! Anda sudah sangat siap. Bantu keluarga dan tetangga untuk meningkatkan kesiapan mereka.'
    )
  }

  if (earthquakeScore < 40 && floodScore < 40) {
    recommendations.push(
      '🚨 PRIORITAS: Segera ikuti pelatihan kesiapsiagaan resmi dari BNPB atau Tagana terdekat!'
    )
  }

  recommendations.push(
    '📱 Install aplikasi SiagaMate AI untuk mendapat peringatan dini bencana real-time.'
  )
...
export function simulateScenario({
  ...
  const templates: Record<string, { title: string; assumptions: string[]; phases: string[] }> = {
    banjir: {
