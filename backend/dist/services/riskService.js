...
const bmkgService_js_1 = require("./bmkgService.js");
/**
 ...
 */
function calculateRiskLevel(magnitude, distanceKm) {
    ...
    if (distanceKm < 50) {
        if (mag >= 6.5)
            return 'high';
        if (mag >= 5.0)
            return 'high'; // M5+ nearby = HIGH RISK
        if (mag >= 4.0)
            return 'medium'; // M4+ nearby = MEDIUM RISK (felt strongly)
 ...
 */
function hasWeatherAlert(weatherSummary) {
 ...
 * - Clear = LOW
 */
function calculateRainfallRisk(weatherCode, humidity, windSpeed) {
    // Heavy rain codes
    ...
    if (weatherCode === '60' || (humidity && humidity > 80))
        return 'low';
    // Default: no rain = low risk
    return 'low';
}
function firstAidByType(disasterType) {
    const playbooks = {
        banjir: [
...
async function predictRisk({ lat, lng, disasterType = 'banjir', profile = 'warga', earthquake: selectedEarthquake, adminLevel4Code, kelurahan, kecamatan, kabupaten, provinsi }) {
    ...
    try {
        ...
        const fetchPromises = [
            (0, bmkgService_js_1.fetchLatestEarthquake)(),
            (0, bmkgService_js_1.fetchWeatherWarningDigest)(),
            (0, bmkgService_js_1.fetchWeatherForecast)(lat, lng)
        ];
        ...
        if (weatherForecastData && weatherForecastData.forecast && weatherForecastData.forecast.length > 0) {
            weatherRiskAnalysis = (0, bmkgService_js_1.analyzeWeatherRisk)(weatherForecastData.forecast);
            weatherForecast = {
