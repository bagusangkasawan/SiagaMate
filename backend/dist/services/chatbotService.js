...
function createUserMessage(message, context) {
    return `Konteks:\n${JSON.stringify(context, null, 2)}\n\nPertanyaan pengguna:\n${message}`;
}
async function askGemini(message, context) {
    if (!env_js_1.env.geminiApiKey) {
        return {
            answer: 'Mode offline aktif. Untuk kondisi darurat: 1) lindungi diri dulu, 2) cek anggota keluarga, 3) evakuasi ke titik aman, 4) pantau BMKG/BPBD, 5) hubungi 112 jika perlu bantuan segera.',
            provider: 'fallback'
        };
    }
    const userMessage = createUserMessage(message, context);
    const response = await axios_1.default.post(`${GEMINI_ENDPOINT}?key=${env_js_1.env.geminiApiKey}`, {
