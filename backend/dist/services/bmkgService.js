...
exports.fetchLatestEarthquake = fetchLatestEarthquake;
exports.fetchFeltEarthquakes = fetchFeltEarthquakes;
exports.fetchRecentEarthquakes = fetchRecentEarthquakes;
exports.fetchWeatherWarningDigest = fetchWeatherWarningDigest;
exports.getAlertsByLocation = getAlertsByLocation;
exports.getBmkgMultiFeed = getBmkgMultiFeed;
exports.fetchBnpbDisasters = fetchBnpbDisasters;
exports.checkNearbyBnpbDisasters = checkNearbyBnpbDisasters;
exports.fetchWeatherForecast = fetchWeatherForecast;
exports.analyzeWeatherRisk = analyzeWeatherRisk;
...
function severityFromMagnitude(magnitudeText) {
...
async function fetchLatestEarthquake() {
    const response = await axios_1.default.get(env_js_1.env.bmkgEarthquakeFeed, { timeout: 8000 });
...
async function fetchFeltEarthquakes(limit = 5) {
    const response = await axios_1.default.get(env_js_1.env.bmkgFeltEarthquakeFeed, { timeout: 8000 });
...
async function fetchRecentEarthquakes(limit = 10) {
    const response = await axios_1.default.get(env_js_1.env.bmkgRecentEarthquakeFeed, { timeout: 8000 });
    const rows = response?.data?.Infogempa?.gempa;
    const list = Array.isArray(rows) ? rows : [];
    return list
        .slice(0, limit)
...
async function fetchWeatherWarningDigest() {
    try {
        const response = await axios_1.default.get(env_js_1.env.bmkgWeatherWarningFeed, {
            timeout: 8000,
            responseType: 'text'
        });
        ...
        return {
            ...
            feed: env_js_1.env.bmkgWeatherWarningFeed
        };
    }
    catch (error) {
        return {
...
function fallbackAlert(lat, lng) {
    return {
        id: `fb-${Date.now()}`,
        title: 'Mode Siaga Aktif',
        severity: 'low',
        message: 'Data BMKG sementara tidak dapat diakses. Gunakan panduan darurat lokal sambil menunggu sinkronisasi.',
        category: 'sistem',
...
async function getAlertsByLocation(lat, lng) {
    try {
...
async function getBmkgMultiFeed() {
 ...
 */
async function fetchBnpbDisasters() {
    try {
        const response = await axios_1.default.get(env_js_1.env.bnpbDisasterApi, {
            timeout: 10000,
            params: {
 ...
 */
function checkNearbyBnpbDisasters(userLat, userLng, disasters, disasterType, distanceThreshold = 100) {
    return disasters.filter((d) => {
        ...
        if (disasterLat === 0 || disasterLng === 0)
            return false;
 ...
 */
function analyzeWeatherRisk(forecast) {
    const indicatorsSet = new Set(); // Use Set for deduplication
